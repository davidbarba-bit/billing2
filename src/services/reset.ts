// Hard reset for a single organization. Wipes ALL data rows for the org
// but **preserves the organization row** (including api_key and NetSuite
// credentials). Counters reset to 0.
...
export async function resetOrganizationData(
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const [customers, services, serviceAddOns, customerAddOns, units, events, invoices, fees, cns, idemRecs] = await Promise.all([
